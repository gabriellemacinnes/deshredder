...
df = pd.DataFrame.from_records(
    records, columns=("dataset", "d", "time", "accuracy (\\%)")
)
...
fp = fp.map(
    sns.lineplot, "d", "accuracy (\\%)", marker="s", ci=None, markersize=8
).add_legend(title="dataset", prop={"size": 16}, labelspacing=0.25)

# fp.ax.set_xscale('log', basex=2)
yticks = [80, 85, 90, 95, 100]
fp.ax.legend_.set_title("\\textbf{dataset}")
handles, labels = fp.ax.get_legend_handles_labels()
fp.ax.legend(
    handles, ["\\textsc{S-Marques}", "\\textsc{S-Isri-OCR}"], ncol=2, columnspacing=1
)
